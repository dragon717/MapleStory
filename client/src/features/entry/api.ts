...
import { apiUrl } from '../../network/endpoints';
...
export async function lobbyRequest<T>(session: LoginResponse, action: string, fields: Record<string, unknown> = {}): Promise<T> {
  const response = await fetch(apiUrl('/api/lobby'), { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: session.token, action, ...fields }) });
