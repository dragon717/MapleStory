...
export const ACTIVE_SKILLS = new Set(['2221045', '2221052', '2221053', '2221054', '1000', '1001', '1002', '2001002', '2001008', '2001009', '2001011', '2001012', '2201001', '2201005', '2201008', '2201009', '2211002', '2211007', '2211011', '2211012', '2211014', '2211017', '2221000', '2221004', '2221005', '2221006', '2221007', '2221008', '2221011', '2221012',
  ...
  // 有按钮才能拖进快捷栏施放。
  '2321005',
  // 復仇天使 `2321054`（2026-09-24 接技能轉換）：与服务端 `world.rs::TRANSFORM_SKILLS`
  // 同一份名单。它是**常驻、不可解除**的转换（源里既没有 `time`、也没有開關文案），
  // 所以**不进** `TOGGLE_SKILLS` —— 按钮始终是「施放」。转换之后慈愛那四本会被服务端
  // 以 `skill_transformed` 拒掉，客户端按返回的提示行显示，不在这里做第二份判断。
  // ⚠️ 被转换出来的復仇副本里，只有 `2301010 天使之觸` 已接执行链（它早就在
  // `BRANCH_AREA_ATTACKS` 里、本集合第 60 行已列），另三本服务端仍回「尚未开放施放」，
  // 因此这里也**不列**它们 —— 列了就会给玩家一个点了必被拒的按钮。
  '2321054',
  // 復甦之光 `2321006`（2026-09-24 接队伍复活）：与服务端 `world.rs::REVIVAL_LIGHT_SKILLS`
  // 同一份名单。它是**主动**技能（源 `action` 是 `resurrectionNew`），复活范围内死亡
  // 的所有队员，再给自己叠源 `time` 秒無敵 —— 有按钮才能拖进快捷栏施放。
  // ⚠️ 它**不是** Hyper 技能（源 `hyper: 0`、`maxLevel 10`、没有 `reqLev`）；本仓文档
  // 里先前的「主教四转 Hyper」是错记。它也不进 `TOGGLE_SKILLS`：源里没有開關文案。
  '2321006',
  ...
  // 且**没有配对消费点**的技能不在这里 ——（S1 自增益窗已接 `1221052 神之滅擊`，
  // 2026-09-24 起 **burn 窗也已接**：`time` 与 `dotTime` 逐级相等的 `1121015 烈焰翔斬`
  // 属「DoT 时长的第二份书写」，按直接伤害 + DoT 接，见下方 `1121015`；
  // 余下带**负面/挑衅** `time` 的波紋衝擊/挑釁契約，孤立 `prop` 的騎士密令/神聖衝擊/
  // 神聖烙印/追隨者衝擊/瞬影殺、孤立 `subTime` 的箭座/暗影霧殺… 共 9 条）与
  // 14 条 `hidden` 节点（合计 23 条）——
  ...
  '1101011', '1111010', '1111012', '1121008', '1121015', '1121052', '1201015', '1211012',
  '1221009', '1221011', '1221052',
