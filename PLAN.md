...
| 地图底部防坠落 | /root/gameplay_backend（Luna max）；已完成 | 修复真实跳跃穿透底层根因；正常地面碰撞优先，越出有效地图下界才按源数据安全落点兜底；保留边缘跨段、Down+Space 与梯顶落地 |
...
- 新版用户入口：http://127.0.0.1:3010/，最新服务PID66754（由一键启动脚本托管），数据库 server/data/qa-gameplay-round2.sqlite3。用户账号与存档保留；本次升级后用户需重新登录。
- 3010 已运行最新后端 binary，启动固定配置为 evidence/runtime/gameplay-round2.json、map-round2.json；贡献EXP、梯绳/下跳、梯顶落地、底部防坠落、复活幂等与怪物AI补丁已部署。
...
- 服务认证session仅在内存，升级已使原用户 token 失效；请重新登录。唯一新版陪测 bot（PID66782）已完成真实认证连接，凭据由脚本保存在 `evidence/runtime/3010-control/bot-credentials.json`（600权限）。旧3000服务及其 bot 已停止，旧数据库 `server/data/accounts.sqlite3` 保留。

## 验收与交接边界

用户已取消独立QA；不创建3011，不重复已有测试，不运行额外网络探针打扰在线用户。梯顶落地与地图底部防坠落均已完成服务端调用链检查、最小定向回归、必要编译并用现有一键脚本受控升级3010。不清库、不改账号存档，短暂重连后交用户刷新、重新登录亲测。复活缺陷已修复并随当前版本部署。
