...
await mkdir(`${evidencePath('inventory-check')}/screenshots`, { recursive: true });
await page.screenshot({ path: `${evidencePath('inventory-check')}/screenshots/inventory-small.png`, fullPage: false });
console.log('small screenshot OK');
await page.click('.inventory-window-button-size');
await page.waitForTimeout(300);
await page.screenshot({ path: `${evidencePath('inventory-check')}/screenshots/inventory-expanded.png`, fullPage: false });
...
await page.screenshot({ path: `${evidencePath('inventory-check')}/screenshots/inventory-window-only.png`, clip: big });
...
await page.screenshot({ path: `${evidencePath('inventory-check')}/screenshots/inventory-top.png`, clip: top });
...
await page.screenshot({ path: `${evidencePath('inventory-check')}/screenshots/inventory-small-full.png`, clip: smallTop });
const smallOnlyHeader = { x: smallTop.x, y: smallTop.y, width: smallTop.width, height: Math.min(smallTop.height, 110) };
await page.screenshot({ path: `${evidencePath('inventory-check')}/screenshots/inventory-small-top.png`, clip: smallOnlyHeader });
