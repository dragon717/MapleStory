...
/// The server only needs the display name.  The source `life`/`hungry`
/// columns stay in `shared/pets.json` for a future hunger loop; they are
/// deliberately not deserialized here because pets are summoned without a
/// persistent instance (see `pets.rs`), so there is nothing for a lifespan
/// or a fullness counter to attach to yet.
#[derive(Clone, Debug, Deserialize)]
#[serde(rename_all = "camelCase")]
