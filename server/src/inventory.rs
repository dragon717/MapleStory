...
pub fn pet_food_fullness(item_id: &str) -> i64 {
    spec_i64(item_id, "incRepleteness")
        .unwrap_or(30)
        .max(0)
}
...
const PET_LEVEL_THRESHOLDS: [i64; 30] = [
    0, 1, 3, 6, 14, 31, 60, 108, 181, 287, 434, 632, 891, 1224, 1642, 2161, 2794, 3555, 4467,
    5542, 6798, 8250, 9950, 11900, 14100, 16600, 19400, 22600, 26100, 30000,
];
...
fn pet_stats(item: &InventoryItem) -> Option<&BTreeMap<String, i64>> {
    is_pet(&item.item_id).then_some(item.stats.as_ref()?).filter(|stats| !stats.is_empty())
}
