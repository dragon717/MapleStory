...
// ── 復甦之光 `2321006`（主教四转队伍复活，2026-09-24 接执行链） ──────────────────
/// 源 `common`：`lt/rb/cooltime/mpCon/time/x/subTime/y/z/w/u`，`action` 是
/// `resurrectionNew`，`info` 里没有 `type`／`massSpell`，`prerequisites` 是空的。
/// 逐字段的口径（源 perLevel：「消耗#mpConMP，**復活在範圍內死亡的所有隊員**後，
/// 獲得#time秒無敵狀態」）：
///
/// ⚠️ **它不是 Hyper 技能**：源 `hyper: 0`、`maxLevel: 10`、`requiredLevel: 0`。
/// 真 Hyper 的形状是 `hyper: 2` ＋ `maxLevel: 1` ＋ `reqLev 140/160/190`
/// （`2321052`／`2321053`／`2321054`／`2321055` 四本都是）。本仓文档先前把它记成
/// 「主教四转 Hyper」，**那是错记**——判据是这三个字段，不是名字。
///
/// 四处落点（第五处不存在）：
///   * **范围 ＝ 同图 ∩ 源矩形**：源 `common.lt` / `common.rb` 是一对 **`vector`**
///     （`lt=(-400,-350)`、`rb=(400,250)`，**绕施法者**），所以要**同时**满足
///     「在同一张图上」与「落在那个矩形里」。框判定走
///     [`skills::World::inside_source_box`]（与 `area_targets_at` 同一套坐标口径），
///     这里不写死任何数值。
///     ⚠️ **别拿 `references/tms273-data/*-source.json` 判 `lt`／`rb`**：那个文件把
///     WZ 的 `vector` 节点**整类丢成 `{}`**（它名下 8 条带框技能**全**是 `{}`，
///     连 `2321005 進階祝福`／`2321008 天怒` 这两个框早就在跑的也一样），照它读就会
///     得出「源没给框」这个**假结论**。要判 `lt`／`rb` 只认导出树的 `rawWz`
///     （`resources/tms273-export/skills.json`，逐节点投影，`_dirType: "vector"`）。
///     门禁 §3j 把这条对照当**事实**钉住：一边是导出树的 vector，一边是那个文件的
///     `{}` —— 后者不得被当作 `lt`／`rb` 的输入。
///     ⚠️ 既有的队伍访问器**不能直接复用**：[`World::party_members_on_map`] 带
///     `player.state.hp > 0` 过滤（它服务的是「分享增益与击杀」），
///     **结构上看不见死人**，而复活要的恰恰是死人 ⇒ 另写
///     [`World::downed_party_members_on_map`]（同图 + `action == "dead"`）。
///   * `time`（秒）⇒ **消费**：窗口内無敵，叠在既有的唯一无敌字段
///     `contact_invulnerable_until` 上（单位是**拍**）。**只给施法者自己**——源那句话的
///     主语是主教（「…後，獲得#time秒無敵狀態」承前省略）；同一段里要带上队员时作者会
///     **显式**写「主教和復活的隊員」（那是 `subTime` 那半），所以「没列出来」＝只有主教。
///   * `subTime`（30 秒内主教与被复活队员 +`x`% 伤害）⇒ **不消费**：它的前提写死在源
///     文案里——「**在有死亡倒數的地圖中**以復甦之光復活時」——而本包**没有**「死亡
///     倒数地图」这个概念（全仓只在这一条源文案里出现过这个词）⇒ 没有可实现的前提。
///     理由带源文案锚点登记在 `check_tms273_damage_pipeline.cjs` 的「復甦之光」段（§3j），
///     那里会**收窄到作用域**反向断言復甦之光自己的执行路径一次都没读过 `sub_time`
///     （⚠️ 不能按全文判：`sub_time` 是冰雪結界 / 冰砾 / 冰瞬移力场共用的一格）。
///   * `x`／`y`／`z`／`w`／`u` ⇒ 只服务 `subTime` 那半（伤害增加量及其智力上限），
///     同一理由不消费。
///   * `lt`／`rb` 的**值**看上面第一条；投影里的 `lt`／`rb` 就是源那对 vector
///     （不是导出器补的默认框 —— 本目录里真正不带框的技能在模型里根本没有 `lt` 键）。
///
/// 冷却走**既有的四转书那一支**（`skills.rs`：`FOURTH_JOB_BOOKS` 成员的 `cooltime`
/// 是**秒**）⇒ `2321006` 不需要新增分支、也不写死字面量（源 L1 `cooltime=1110`、
/// L10 `cooltime=300`）。
const SKILL_REVIVAL_LIGHT: u32 = 2321006;
/// 復甦之光的接纳表。**判据只有这一处**：施法白名单、施法臂与动作时长都读它。
...
const SKILL_GODS_ANNIHILATION: u32 = 1221052;
