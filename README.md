...
默认简体中文，页面语言选择器可切换 English 并记住选择；`?lang=zh` / `?lang=en` 可覆盖保存的偏好。273 的地图、NPC、道具及任务显示文本使用 [OpenCC](https://github.com/nk2028/opencc-js) 繁转简，原始资料和协议标识不改写。源 PNG 内嵌文字仍为原版繁体；原版任务资料缺少英文翻译时保留源文。
