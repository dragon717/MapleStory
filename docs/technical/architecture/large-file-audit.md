...
> 对应 `docs/plan/topics/MapleStory_Large_File_Refactoring_Plan.md` §3。
...
| 文档 | 行数 | churn | 性质 | 处置建议 |
| --- | --- | --- | --- | --- |
| `docs/plan/PLAN.md` | 1,193 → **已拆** | **29/100** | 当前任务入口（只保留待办与待确认） | **已完成**（见 §6.1）。当前入口为 `docs/plan/PLAN.md`；原已完成条目按日期拆入 `docs/plan/history/YYYY-MM-DD/`，由 `docs/plan/INDEX.md` 导航 |
| `IMPLEMENTATION_STATUS.md`（迁移前） | 738（预警） | 16/100 | 历史台账 | 已按真实日期/任务拆入 `docs/plan/history/YYYY-MM-DD/`，索引统一放在 `docs/plan/INDEX.md`，不保留第二份台账 |
| `docs/plan/topics/MapleStory_Rust_Chat_Ops_Development_Plan.md` | 1,379（需解释） | 1/100 | 有效专题实施方案 | 整体归入 `docs/plan/topics/`，不另建历史副本 |
| `docs/design/world-awareness/*.md` | 890–1,095 | 0/100 | 有效设计文档 | 设计类文档天然偏长，**按内容分节**即可，不按行数机械拆 |
| `docs/plan/topics/bugfix/*.md` | 311–963 | 0–1/100 | 未完成问题方案 | 保留为专题入口；已完成/阶段快照归入 `docs/plan/history/` |

**本轮已按同一口径产出**：`docs/technical/architecture/`（3 篇，每篇只讲一件事）、`docs/technical/dev/current-entry-audit.md`、
...
| 拆分后 `docs/plan/PLAN.md` | 当前入口只留仍需行动/确认事项、R11 后续和最近一批实玩清单 + 索引指针 |
| 归档 | `docs/plan/history/` 下 **165 篇**（66 篇旧 PLAN 记录、18 篇本轮 PLAN 完成项、79 篇 IMPLEMENTATION_STATUS 条目、2 篇 bugfix 快照），按 `YYYY-MM-DD/` 分目录 |
| 保真度 | 每篇 = 原标题 + 原有正文逐字 + 3 行来源标注（归档自 PLAN.md / 归档位置 / 归档顺序），**正文零改写**（合计 1,192 行原文） |
| 索引 | `docs/plan/INDEX.md`：只列日期、任务、状态和文件链接，正文仍在对应日期目录 |
