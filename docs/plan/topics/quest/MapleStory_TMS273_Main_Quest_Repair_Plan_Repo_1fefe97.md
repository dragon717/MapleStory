...
这是一份技术证据清单，不取代 `docs/plan/PLAN.md` 的当前任务状态台账。
...
本文件是专题设计交付。正式采用后，在 `docs/plan/PLAN.md` 登记当前范围、负责人、依赖与验收；完成证据进入 `docs/plan/INDEX.md`，不复制第四份进度台账。[R01][R02]
...
**本轮仅写计划，不触发重启。** 实施后的运行加载按现有 `docs/plan/PLAN.md` 约定，待用户批准时机再使用统一入口。HMR/开发模式统一入口是另一个已有任务，不绑成本修复的前置重构。[R01][R10]
...
- **[R01] 工作规则与当前计划**：`AGENTS.md`、`docs/plan/PLAN.md`。  
...
- **[R02] 长期业务与验收规范**：`docs/technical/BUSINESS_DEVELOPMENT.md`。  
