...
| `docs/technical/dev/current-entry-audit.md` | 已有静态 P0 取证，并明确未启动验证 | 更新该文件中的过时判断，不新建重复台账 |
...
继续使用 `runtime/3010-control/`，在现有 server / bot 信息之外增加 `vite.pid`、`vite.log` 和实例描述；这些运行态文件继续不入 Git。[R01][R15]
...
| 阶段 | 范围 | 主要文件 | 完成门槛 |
| --- | --- | --- | --- |
| P0 修订 | 纠正已有取证与首版假设 | 已有 HMR 专题计划、`docs/technical/dev/current-entry-audit.md`、`docs/plan/PLAN.md` | 现状 / 建议 / 未测分开；不重做无关历史 |
...
将本计划作为现有 `docs/plan/topics/MapleStory_TMS273_Web_Realtime_HMR_Repo_Reviewed_v2.md` 的新版本内容，或由该文件明确链接到本版；不要保留两份互相冲突的“当前推荐”。

`docs/plan/PLAN.md` 只记录正在实施的阶段、依赖、待用户验收项。更新现有 P0 文档的端口选择和资源链错误，保留其“当时没有运行实测”的边界。已完成结果按原规范记录到 `docs/plan/INDEX.md`，不新增第四份重复状态台账。[R10][R13][R17]
...
输入：本计划、当前实际工作区、`docs/technical/BUSINESS_DEVELOPMENT.md` 与 `docs/plan/PLAN.md`。
...
- [R10] `docs/technical/BUSINESS_DEVELOPMENT.md`：权威、资源、开发验证与文档治理规范。
- [R11] `client/src/features/world/water.ts`：Phaser 水面与独立表现模拟边界。
- [R12] `scripts/assemble_tms273.cjs`：实际资源写入目标、shared 输出。
- [R13] `docs/technical/dev/current-entry-audit.md`：已有 P0 取证及需修正的描述。
...
- [R17] `docs/plan/PLAN.md`：当前 HMR 待办、未完成运行验收与其他待确认事项。
