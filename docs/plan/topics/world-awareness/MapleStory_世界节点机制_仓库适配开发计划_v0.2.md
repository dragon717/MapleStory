...
> 文档定位：本文件是世界节点的专题规格，不是第四份执行台账。当前任务登记仍在 `docs/plan/PLAN.md`；完成证据仍进入 `docs/plan/INDEX.md`。
...
当前默认仍由 `启动3010.command` 构建后重启，`docs/plan/PLAN.md` 中的 HMR 默认行为变更仍待用户决定。世界节点不借机切换默认开发服务器、不改端口，不把修改源码当作已加载到 `dist-tms273`。[R01][R13]
...
以下是专题工作包，不维护实际进行中状态；实施时将当前一张卡及其依赖登记到 `docs/plan/PLAN.md`。
...
本计划没有启动子代理，也没有修改 `docs/plan/PLAN.md`。实际执行时按仓库治理记录负责人和验证证据，不虚构已派发任务。[R01]
...
建议实施时在 `docs/plan/PLAN.md` 只登记一条活动入口，示例如下；负责人等执行信息由实际实施过程填写：
