...
function displayText(text2) {
  return locale === "zh" ? simplify(text2) : text2;
}
...
var EntryView = class {
  ...
  async loadArt() {
    try {
      ...
      this.setNote(error instanceof Error ? error.message : String(error), true);
    }
  }
  showLogin() {
    this.revision++;
  ...
  async refreshCharacters() {
    const result = await lobbyRequest(this.session, "list");
  ...
  setNote(note, error = false) {
    this.note = note;
    this.error = error;
    const target = this.host.querySelector(".entry-notice");
    if (target) {
  ...
  setBusy() {
    ...
    this.host.querySelectorAll("button").forEach((button) => {
      button.disabled = this.busy || button.dataset.unavailable === "true";
    });
  }
  go(stage) {
    this.stage = stage;
  ...
  render() {
    ...
    document.body.classList.add("entry-active");
    this.host.innerHTML = `<div class="entry-background" aria-hidden="true"></div><div class="entry-brand">MapleStory <small>TMS 273.7</small></div><div class="entry-scene">${this.stage === "login" ? this.loginMarkup() : this.stage === "channel" ? this.channelMarkup() : this.stage === "characters" ? this.charactersMarkup() : this.createMarkup()}</div><nav class="entry-navigation" aria-label="${text("\u5927\u5385\u5BFC\u822A", "Lobby navigation")}">${this.stage !== "login" ? `<button type="button" data-action="first">\u2039 ${text("\u9996\u9875", "FIRST")}</button><button type="button" data-action="back">\u2039 ${text("\u8FD4\u56DE", "BACK")}</button>` : ""}</nav><p class="entry-notice${this.error ? " entry-error" : ""}" role="status"${this.note ? "" : " hidden"}>${escape(this.note)}</p><div class="entry-edition">TMS 273.7 \xB7 ${text("\u4E3B\u9891\u9053", "Main channel")}</div>`;
    this.renderBackground();
    this.bind();
    this.renderOptions();
    this.renderPreviews();
  ...
  worldBadge() {
    return `<div class="entry-world-badge entry-glass"><span class="entry-world-emblem">\u{1F341}</span><div>${text("\u5192\u9669\u5C9B", "Maple World")}<small>CH. 1 \xB7 ${text("\u4E3B\u9891\u9053", "Main channel")}</small></div></div>`;
  }
  channelMarkup() {
  ...
  charactersMarkup() {
    ...
    const slots2 = Array.from({ length: Math.min(12, this.slotLimit - start) }, (_, index) => {
      const character = this.characters[start + index];
      return character ? `<button class="entry-character${character.id === this.selected ? " selected" : ""}" data-select="${character.id}" aria-pressed="${character.id === this.selected}"><div class="entry-avatar" data-character="${character.id}"></div><strong>${escape(character.name)}</strong><span>Lv. ${character.level} \xB7 ${jobName(character.job)}</span></button>` : `<button class="entry-character entry-empty" data-action="create" aria-label="${text("\u521B\u5EFA\u89D2\u8272", "Create character")}"><div class="entry-avatar" data-empty="true"></div><span>\uFF0B</span></button>`;
    }).join("");
    return `<aside class="entry-selection-info">${this.worldBadge()}<p class="entry-slots">${text("\u89D2\u8272\u680F\u4F4D", "CHARACTER SLOT")} <strong>${this.characters.length}/${this.slotLimit}</strong></p></aside><section class="entry-character-stage" aria-label="${text("\u89D2\u8272\u9009\u62E9", "Character selection")}"><div class="entry-character-grid">${slots2}</div><div class="entry-pages">${Array.from({ length: Math.ceil(this.slotLimit / 12) }, (_, index) => `<button data-page="${index}" class="${index === this.page ? "selected" : ""}" aria-label="${text("\u7B2C", "Page ")}${index + 1}${text("\u9875", "")}">${index + 1}</button>`).join("")}</div><div class="entry-character-actions"><button class="entry-tan" data-action="create"${this.characters.length >= this.slotLimit ? ' data-unavailable="true"' : ""}>\uFF0B ${text("\u521B\u5EFA\u89D2\u8272", "Create Character")}</button><button class="entry-primary" data-action="enter"${!this.selected ? ' data-unavailable="true"' : ""}>${text("\u5F00\u59CB\u6E38\u620F", "PLAY")}</button></div></section>`;
  }
  createMarkup() {
  ...
  bind() {
    this.host.querySelector("#login")?.addEventListener("submit", (event) => {
      event.preventDefault();
      void this.run(async () => {
        const username = this.host.querySelector("#username").value.trim();
        const password = this.host.querySelector("#password");
        const save = this.host.querySelector("#save-id").checked;
        this.session = await authenticate(username, password.value, this.register);
        password.value = "";
        try {
          if (save) localStorage.setItem("maple-saved-id", username);
          else localStorage.removeItem("maple-saved-id");
        } catch {
        }
        this.register = false;
        await this.refreshCharacters();
    ...
    this.host.querySelector("#mode")?.addEventListener("click", () => {
      this.register = !this.register;
      this.render();
    });
    this.host.querySelector("#character-name")?.addEventListener("input", (event) => {
    ...
    this.host.querySelector("#create-character")?.addEventListener("submit", (event) => {
      event.preventDefault();
      void this.run(() => this.create());
    });
    this.host.querySelectorAll("[data-action]").forEach((button) => button.addEventListener("click", () => this.action(button.dataset.action)));
    this.host.querySelectorAll("[data-select]").forEach((button) => {
      ...
      button.addEventListener("dblclick", () => {
        this.selected = button.dataset.select;
        void this.run(() => this.startGame());
      });
    });
    this.host.querySelectorAll("[data-page]").forEach((button) => button.addEventListener("click", () => {
  ...
  action(action) {
    ...
    if (action === "language") {
      ...
      url.searchParams.set("lang", uiLocale() === "en" ? "zh" : "en");
      location.assign(url);
      return;
    }
    ...
    if (action === "create") {
      if (this.characters.length >= this.slotLimit) return;
      this.draftName = "";
      this.checkedName = "";
      this.createRequestId = "";
      this.go("create");
    ...
    if (action === "check-name") return void this.run(async () => {
      const name = this.draftName.trim();
      const result = await lobbyRequest(this.session, "checkName", { name });
      this.checkedName = result.available ? name : "";
      this.setNote(result.available ? text("\u6B64\u540D\u79F0\u53EF\u4EE5\u4F7F\u7528\u3002", "This name is available.") : text("\u6B64\u540D\u79F0\u5DF2\u88AB\u4F7F\u7528\u3002", "This name is taken."), !result.available);
    });
  ...
  async create() {
    ...
    const result = await lobbyRequest(this.session, "create", { name, appearance: this.appearance, requestId: this.createRequestId });
    await this.refreshCharacters();
  ...
  async startGame() {
    ...
    const session = await lobbyRequest(this.session, "select", { characterId: this.selected, channelId: 1 });
    try {
  ...
  renderBackground() {
    const target = this.host.querySelector(".entry-background");
  ...
  renderOptions() {
    const target = this.host.querySelector(".entry-appearance-options");
    ...
    const rows = [["face", "\u8138\u578B", "FACE"], ["hair", "\u53D1\u578B", "HAIR"], ["coat", "\u670D\u88C5", "COSTUME"], ["shoes", "\u978B\u5B50", "SHOES"], ["weapon", "\u6B66\u5668", "WEAPON"]];
    ...
    target.querySelectorAll("[data-gender]").forEach((button) => button.addEventListener("click", () => {
      this.chooseGender(Number(button.dataset.gender));
      this.renderOptions();
      this.renderPreviews();
    }));
    ...
    target.querySelectorAll("[data-hair-color]").forEach((button) => button.addEventListener("click", () => {
      this.appearance.hair = Number(button.dataset.hairColor);
      this.createRequestId = "";
      this.renderOptions();
      this.renderPreviews();
    }));
...
function groupMenuEntries(entries) {
  const groups = Array.from({ length: CATEGORY_X.length }, () => []);
  for (const entry2 of entries) {
    const keyColumn = Number(entry2.key.split("/")[2]);
    const sourceColumn = Number.isFinite(entry2.x) ? Math.round((entry2.x - CATEGORY_X[0]) / 140) : 0;
    const column = Number.isInteger(keyColumn) && keyColumn >= 0 && keyColumn < groups.length ? keyColumn : Math.max(0, Math.min(groups.length - 1, sourceColumn));
    groups[column].push(entry2);
  }
  return groups;
}
var MenuView = class {
  constructor(host, manifest, status, onInventory, onQuit, onEquipment, onQuest, onSkills, onCharacterInfo, onChannel, onCharacters, onSettings, onNews, onParty, onFriend, onEmoticon, onActivities, onWorldMap, onCashShop, onKeybindings) {
    ...
    this.onCharacterInfo = onCharacterInfo;
  ...
  createMenu(kind, assets) {
    ...
    for (const [column, entries] of groups.entries()) {
      ...
      for (const entry2 of entries) {
        const normal = assets[`${entry2.key}/normal/0`];
        if (!normal) continue;
        const button = this.createAssetButton(entry2.key, displayText(entry2.label), assets, "maple-menu-item", normal);
        button.dataset.menuItem = entry2.key;
        button.setAttribute("role", "menuitem");
        button.style.setProperty("--menu-entry-y", `${entry2.y}px`);
        button.addEventListener("click", () => this.activateEntry(entry2));
        items.append(button);
      }
  ...
  createOperation(operation, assets) {
  ...
  activateEntry(entry2) {
    ...
    if (action) {
      ...
      this.status(displayText(`${entry2.label}\u5C1A\u672A\u5B9E\u88C5\u3002`));
    }
  }
  positionMenu(menu) {
    ...
    const narrow = hostWidth < 1e3 || hostHeight < WIDE_MENU_HEIGHT_MIN;
  ...
  handleMenuKeyDown(event, menu) {
    ...
    candidates.sort((a2, b2) => {
      const ar = a2.getBoundingClientRect(), br = b2.getBoundingClientRect();
  ...
  createAssetButton(key, label, assets, className, normal) {
    const button = document.createElement("button");
    button.type = "button";
    ...
    this.bindButton(button, button.firstElementChild, assets, key, normal);
    return button;
  }
  bindButton(button, image, assets, key, normal) {
...
// <stdin>
var entry = new EntryView(document.getElementById("welcome"), async (session) => {
  document.getElementById("entered").textContent = session.username;
});
document.getElementById("show-menu").onclick = async () => {
  const manifest = await fetch("/assets/manifest.json").then((r2) => r2.json());
