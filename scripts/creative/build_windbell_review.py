...
OUT = ROOT / 'resources/ui/windbell'
...
def url(path):
    return quote(Path(os.path.relpath(path, OUT)).as_posix())

def build():
    scene_images = sorted(p for p in (SCENES / 'images').glob('*.png') if not any(tag in p.stem for tag in ('-study-', '-clean-edit-')))
    scene_images = [(SCENES / 'images/clean' / p.name) if (SCENES / 'images/clean' / p.name).exists() else p for p in scene_images]
    character_images = sorted(CHARACTERS.joinpath('images').glob('*.png'))
    character_images = [(CHARACTERS / 'images/clean' / p.name) if (CHARACTERS / 'images/clean' / p.name).exists() else p for p in character_images]
    images = scene_images + character_images
    renders = sorted((BLENDER / 'renders').glob('*.png')) if (BLENDER / 'renders').exists() else []
    music = sorted(MUSIC.rglob('*.ogg')) if MUSIC.exists() else []
    effects = sorted(SFX.glob('*.ogg')) if SFX.exists() else []
    models = sorted(p for p in BLENDER.rglob('*') if p.suffix in ('.blend', '.glb')) if BLENDER.exists() else []
    narratives = sorted((SCENES / 'narrative').glob('*.json')) if (SCENES / 'narrative').exists() else []
    cards = []
    for path in images + renders:
        if path.stem in ('island-ancient-tree', 'island-floating-ground'):
            group = '2D 新稿 · 图像生成'
        else:
    ...
    for path in effects:
        cards.append('<article data-kind="audio"><small>原创事件音效 · OGG</small><h3>{n}</h3><audio controls preload="none" src="{u}"></audio><a href="{u}" download>保存音频 ↗</a></article>'.format(u=url(path), n=esc(path.stem)))
    for path in models:
...
<header><small>WINDBELL / ORIGINAL WORLD STUDIES</small><h1>风经过，<br>这里记得。</h1><p>风铃桥的公共修复，与风铃岛的自由抵达。这里展示实际已生成的场景、模块、人物、声音和叙事数据；制作中缺失的文件不会显示为完成。</p><p><strong>交付边界：</strong>原创 P 类资产与设计包；活动清单及服务端地图交互已完成候选构建，在线 3010 尚未切换。主视觉用于参考，碰撞由共享地图几何定义；NPC 使用静态立绘。图像由用户同意的内置工具生成，工具未暴露准确模型名。</p><nav aria-label="筛选交付类型"><button data-filter="all" aria-pressed="true">全部</button><button data-filter="image" aria-pressed="false">场景与美术</button><button data-filter="audio" aria-pressed="false">音乐与音效</button><button data-filter="model" aria-pressed="false">3D 源文件</button><button data-filter="narrative" aria-pressed="false">文案与行为树</button></nav></header><main><div class="grid">'''+''.join(cards)+'''</div></main><footer>资源保存在当前项目 resources/scenes、resources/characters、resources/ui、resources/music、resources/sfx 与 resources/blender。请点击图像查看原尺寸；音频只在你按播放时发声。<a href="manifest.json">制作清单</a> · <a href="../../../docs/design/windbell/PRODUCTION.md">设计规格</a></footer><script>document.querySelectorAll('[data-filter]').forEach(b=>b.addEventListener('click',()=>{document.querySelectorAll('[data-filter]').forEach(x=>x.setAttribute('aria-pressed',String(x===b)));document.querySelectorAll('[data-kind]').forEach(x=>x.hidden=b.dataset.filter!=='all'&&x.dataset.kind!==b.dataset.filter)}));</script></html>'''
