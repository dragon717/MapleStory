...
# 4=etc).  All are inventoryType 2.  The "choose" (-1) coupons are authored as
# NPC-script driven and share a non-own icon via UOL; they are intentionally
# omitted this round because the selection UI and script do not exist yet.
COUPONS = {
    '2430768': 1,
    '2430769': 2,
    '2430770': 3,
...
def tms_item(item_id: str):
    """Return (item_dict, sprite_group) for one coupon, or None.

    `sprite_group` is the plain 4-digit WZ `.img` group: a 263-series item
    lives in `0263.img`, never `0243.img`, so the sprite path is derived from
    the id rather than the JSON directory.
    """
    padded = f'{int(item_id):08d}'
    base = WZ / 'Consume'
    group = padded[:4]
    # Split client packs sometimes keep the newer `0243 2` sibling directory
    # (with a space), exactly like `export_tms273.cjs` handles the sprite
    # fallback.  Try the plain group dir first, then the spaced sibling.
    candidates = [base / group, base / f'{group} 2']
    for path in candidates:
        candidate = path / f'{padded}.json'
        if candidate.exists():
            break
    else:
        return None
...
def tms_sprite(item_id: str):
    wrapped = tms_item(item_id)
    return f'Item/Consume/{wrapped[1]}.img/{int(item_id):08d}/info/icon' if wrapped else None
...
def main() -> None:
    ...
    for item_id, slot_expand in COUPONS.items():
        wrapped = tms_item(item_id)
        if wrapped is None or wrapped[0]['slotExpand'] != slot_expand:
            raise SystemExit(f'{item_id}: TMS273 slotExpand mismatch ({wrapped})')
        src, sprite_group = wrapped
        ...
        entry = {
            ...
            'spec': {},
            'source': f'Item/Consume/{sprite_group}/{item_id}.json',
            'sourceItemId': f'{int(item_id):08d}',
            'spriteSource': f'Item/Consume/{sprite_group}.img/{int(item_id):08d}/info/icon',
