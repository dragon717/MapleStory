...
# `221000002`/`221000301 某處`（无静态入口）、
# `220000301..220000307` 村莊民宅（7 间内景）。
# 本部 `west00` 之后的危險地帶/UFO 街由 ADDITIONAL_UFO_MAP_IDS 承接（见下）。
...
# 危險地帶／洛斯威爾草原／UFO（地球防衛總部街的下一段，2026-09-15，
# 审计 T06/交通 下一片）。上一片的「不装」注释点名的 `221030000` 与
# `22103xxx` 就在本片，`22104xxx` 经全量核对在 TMS273 WZ 中并不存在。
#
# 源事实（TMS273.7 WZ 全量核对，Map/Map/Map2 + Graph.json）：
# 本部 `221000000.west00` → `221030000 危險地帶` 的 `east00`；地面链是四条
# 纯静态门 `221030000 ⇄ 100 ⇄ 200 ⇄ 300 ⇄ 400`（洛斯威爾草原Ⅰ~Ⅳ，
# 怪 4230127 馬堤安 / 4230128 培利堤安，NPC 2052018/2052019 波特博士）。
#
# UFO 內部（`2210305xx`~`2210307xx`）同样以源静态门为主：`221030520 走廊102`
# 往西 `221030510 走廊101`（其上 `west00` 通 `221030500 德秋的房間`）、往东
# `221030530 走廊103`（`up00 ⇄ 221030630 走廊204` 下到 `221030530`）；东侧
# 接 `221030540 走廊104`、`221030600 走道201 ⇄ 610 ⇄ 620 ⇄ 630` 与
# `221030640 走廊H01 ⇄ 650 ⇄ 660`（一条回头链）。出 UFO 走
# `221030520.pt00 → 221030400.pt00`（源静态门，另一端就是进场的落点）。
#
# 只有四道门在源里带脚本名、需要 P 级路由（全部有 Graph.json 授权）：
#   1. `221030400`（源 `Graph.json` portalNum 9999 / `scriptPortal: 2052026`）
#      —— 该图 NPC `2052026 UFO呼叫器` 是唯一的进场方式，目标是 `221030520`。
#      NPC 站位 x=-1938 与该图 `pt00`（pt:1，y=100）重合，原版由脚本体传送。
#   2. `221030540.pt00`（pt:7，`pt_221030540`，portalNum 2）→ `221030550`
#      `走廊105`；该图 `west00` 是源静态门，回 `221030540.pt00`。
#   3. `221030550.pt00`（pt:7，`pt_221030550`，portalNum 4）→ `221030551`
#      `走廊105`（同图另一段）。
#   4. `221030600.up00`（pt:7，`pt_221030600`，portalNum 4）→ `221030700`
#      `通風口 D-1`，其 `west00` 是源静态门回 `221030600.up00`；
#      `700 ⇄ 710 ⇄ 720 ⇄ 730` 全静态。
# `221030550.col00`（pt:9，`col_221030550`）在 Graph.json 的授权目标是
# 999999999（未授权）⇒ 保持未开放，本片不接。
#
# 不装（源里没有任何授权入边，装了是死端）：`221030801/802/803/804/805/840`
# （`UFO操縱桿某處`）、`221030850`/`221030890`（连 `String/Map.json` 都没有
# 名字）、`221030621`（`地球防衛總部某處`）、`221030821`（同上，`pt00` 未授权）、
# `221030731`（`換氣孔D-4`）、`221030701`（`UFO換氣孔某處`）、`221030541`
# （`走廊104` 另一段）、`221030602`（`走廊201` 另一段）、整片 `操縱杆` 翼
# `221030800/810/820/821/850`（Graph 授权只指向翼内与 `221030730`，没有任何
# 图指向它们 ⇒ 入口是原版腳本，本地缺失）。`221030900 主控室` 与
# `221030910 通風口 D-77` 是 `BossCaoong` 首領房（`info.lvLimit` 180、
# `fieldScript BossCaoong`、`BPReturn_Caoong`/`pt2_221030910` 出入），
# 本仓库没有该首領 ⇒ 按边界不装。
# `221030301 馬堤安草原`/`221030401 培利堤安草原`/`221030501 梅卡堤安草原`/
# `221030601 堤安之地` 在 `String/Map.json` 有名字，但 TMS273 WZ **没有对应的
# `.img`**（已按 id 反查 Map2 分卷确认）⇒ 源边界，不是漏装。
ADDITIONAL_UFO_MAP_IDS = (
